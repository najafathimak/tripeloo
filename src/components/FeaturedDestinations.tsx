...
type Card = { slug: string; name: string; image: string; location?: string };
...
async function fetchFeatured(): Promise<Card[]> {
  try {
    ...
    return rows.slice(0, 4).map((d: any) => ({
      slug: d.slug,
      name: d.name,
      image: d.coverImage,
      location: d.location ?? d.name, // fallback in case location is missing
    }));
  } catch {
    return fallback.slice(0, 4).map((d) => ({
      slug: d.slug,
      name: d.name,
      image: d.coverImage,
          ...
          {destinations.map((d) => (
            <Link
              ...
              href={{
                ...
                query: { destination: d.location || d.name },
              }}
